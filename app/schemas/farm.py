...
class GeoJSONPoint(BaseModel):
    ...
    type: str = Field(..., description="GeoJSON type (expected 'Point')")
...
class GeoJSONPolygon(BaseModel):
    ...
    type: str = Field(..., description="GeoJSON type (expected 'Polygon')")
    coordinates: List[List[List[float]]] = Field(..., description="Array of linear rings")


class FarmAreaRequest(BaseModel):
    """Request body for computing farm area from a boundary polygon."""
    boundary: GeoJSONPolygon = Field(..., description="Farm boundary as GeoJSON Polygon")


class FarmAreaResponse(BaseModel):
    """Response containing computed farm area in multiple units."""
    area_sq_meters: Decimal = Field(..., description="Area in square meters")
    area_hectares: Decimal = Field(..., description="Area in hectares")
    area_acres: Decimal = Field(..., description="Area in acres")


class FarmMapData(BaseModel):
    """Map-ready data for a farm: geometry and area.

    Designed for frontends that need a single payload containing the point
    location, boundary polygon, centroid and area values.
    """
    farm_id: str
    name: str
    ...
    centroid: Optional[Dict[str, float]] = Field(None, description="Centroid of boundary as {lat, lon}")
    area_sq_meters: Optional[Decimal] = Field(None, description="Area in square meters (if available)")
    area_hectares: Optional[Decimal] = Field(None, description="Area in hectares (if available)")
    area_acres: Optional[Decimal] = Field(None, description="Area in acres (if available)")
...
class FarmAttributes(BaseModel):
    ...
    water_ph: Optional[float] = Field(None, description="Water pH level")
...
class FarmCreate(BaseModel):
    ...
    address: Optional[str] = Field(
        None,
        description="Street address or village name (if provided, cannot be blank)",
    )
    district: Optional[str] = Field(
        None,
        description="District name (e.g., Chennai, Coimbatore)",
    )
    state: Optional[str] = Field(
        None,
        description="State or region (e.g., Tamil Nadu)",
    )
    pincode: Optional[str] = Field(
        None,
        description="6-digit postal code (Indian PIN code)",
    )
