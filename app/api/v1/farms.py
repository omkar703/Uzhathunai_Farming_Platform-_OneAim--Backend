...
@router.get("/{farm_id}/map", response_model=FarmMapData)
def get_farm_map_data(
    farm_id: UUID,
    current_user: User = Depends(get_current_active_user),
    db: Session = Depends(get_db),
):
    """Get map-ready geometry and area information for a farm.

    This endpoint accepts a farm identifier and returns a single, consistent
    payload that front-end applications can use directly to render the farm
    on a map. The response includes:

    - the stored location point as GeoJSON (WGS84)
    - the stored boundary polygon as GeoJSON (WGS84), when available
    - the centroid of the boundary (latitude/longitude), when a boundary exists
    - the farm area expressed in square meters, hectares, and acres, based on
      the value persisted for the farm

    Access to this endpoint is restricted to authenticated users within the
    same organization that owns the farm.
    """
    service = FarmService(db)

    # Get user's current organization (same pattern as other farm endpoints)
    from app.models.organization import OrgMember
    from app.models.enums import MemberStatus
    ...
    # Reuse existing service method so we respect org-ownership rules
    farm = service.get_farm_by_id(farm_id, membership.organization_id)

    # Compute centroid if we have a boundary
    centroid = None
    if farm.boundary:
        spatial = SpatialService(db)
        centroid = spatial.get_centroid(farm.boundary)

    area_sq_meters = farm.area
    area_hectares: Optional[Decimal] = None
    area_acres: Optional[Decimal] = None
    if area_sq_meters is not None:
        area_hectares = area_sq_meters / Decimal("10000")
        area_acres = area_sq_meters / Decimal("4046.8564224")
